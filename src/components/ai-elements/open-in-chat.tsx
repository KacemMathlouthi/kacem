...
export const OpenInGrok = (props: OpenInGrokProps) => {
  const { query } = useOpenInContext();
  return (
    <DropdownMenuItem {...props} render={<a className="flex items-center gap-2" href={providers.grok.createUrl(query)} rel="noopener" target="_blank" />}><span className="shrink-0">{providers.grok.icon}</span><span className="flex-1">{providers.grok.title}</span><ExternalLinkIcon className="size-4 shrink-0" /></DropdownMenuItem>
  );
};
