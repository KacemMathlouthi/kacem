...
type Intersection2Props = {
  children: ReactNode
  /** Overrides the content column width; defaults to the home page's. */
  className?: string
}

const Intersection2 = ({ children, className }: Intersection2Props) => (
  <div className="relative grid min-h-svh w-full grid-cols-[1fr_1rem_minmax(0,auto)_1rem_1fr] grid-rows-[1fr_1px_auto_1px_1fr] [--pattern-fg:var(--color-gray-950)]/5 dark:bg-transparent dark:[--pattern-fg:var(--color-white)]/10">
    <div
      className={cn(
        "relative col-start-3 row-start-3 flex max-w-lg flex-col lg:max-w-4xl",
        className
      )}
    >
      {children}
